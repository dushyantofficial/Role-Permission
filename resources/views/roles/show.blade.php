    ...
    <main class="app-content {{user_theme_get()}}">
        <div class="app-title">
            ...
            <div class="pull-right">
                <a class="btn btn-sm btn-shadow btn-outline-secondary btn-hover-shine" href="{{ route('roles.index') }}"> Back</a>
            </div>
        </div>
            ...
            <div class="form-group">
                ...
                @if(!empty($rolePermissions))
                    @foreach($rolePermissions as $index =>$permission)
                        <label class="badge" style="background-color: {{$randomColor}}">{{ $permission->name }}</label>
                        @if(($index + 1) % 6 == 0)
                            <br>
                        @endif
                    @endforeach
